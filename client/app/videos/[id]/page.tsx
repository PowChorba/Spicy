...
export default async function VideoWatch({ params }: any) {
  ...
  return (
    <>
      <Navbar />
      <div className="flex">
      <article className="w-5/6 py-2">
      <iframe
        src={renderVideo[0].url}
        allowFullScreen
        className="w-full h-[600px] border-black border-2 rounded-lg"
      ></iframe>
      <div className="py-2">
        ...
        <div className="py-2 border-t-2 border-black flex gap-2">
            <h5>Categorias:</h5>
            {
              categoryFilter.map((category: CategoryFormat) => {
                return <Link href={`/categorias/${category.idCategory}`} key={category._id} className="border-2 border-[#D63423] bg-[#D63423] text-white px-2 rounded-lg">{category.name}</Link>
            })}
        </div>
      ...
      <aside className="w-1/6 text-center p-2">
