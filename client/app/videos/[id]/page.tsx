...
export default async function VideoWatch({ params }: any) {
  ...
  return (
    <>
      <Navbar />
      <div className="flex">
      <article className="w-5/6 py-2 max-sm:w-full max-sm:px-1">
      <iframe
        src={renderVideo[0].url}
        allowFullScreen
        className="w-full h-[600px] border-black border-2 rounded-lg max-sm:h-[350px]  overflow-hidden"
      ></iframe>
      <div className="py-2">
        <h3 className="font-semibold text-2xl max-sm:text-lg">{renderVideo[0].title}</h3>
      ...
      <aside className="w-1/6 text-center p-2 max-sm:hidden">
