...
export default function CategoryRender({data, videos}: Props){
    ...
    return(
        ...
        <button onClick={handlePrev} className={catRender[0].name !== dataUno[0].name ? "p-2  text-lg rounded-md " : 'invisible'}>{"<"}</button>
        ...
        <button onClick={handleNext} className={catRender[0].name !== dataDos[0].name ? "p-2  text-lg rounded-md " : 'invisible'}>{">"}</button>
        </div>
